...
const abiTeam = require('../../contracts/Team.sol/Team.json').abi;
const abiTeamFactory = require('../../contracts/TeamFactory.sol/TeamFactory.json').abi;
...
const verifySignature = async (message, signature, address) => {
  try {
    ...
    const signAddress = web3.eth.accounts.recover(prefix + message, signature);

    return signAddress.toLocaleLowerCase() === address.toLocaleLowerCase();
  } catch (e) {
    return false;
  }
}
...
const getLatestTeamNftContentUris = async (teamAddress) => {
  const teamContract = new web3.eth.Contract(
    abiTeam,
    teamAddress,
  );
  const getLatestTeamNftContentUris = await teamContract.methods.getLatestTeamNftContentUris().call();
  return getLatestTeamNftContentUris;
}

const getTeamAddress = async (teamId) => {
  const teamFactoryContract = new web3.eth.Contract(
    abiTeamFactory,
    process.env.TEAM_FACTORY_ADDRESS,
  );
  const getTeamAddress = await teamFactoryContract.methods.getTeamAddress(teamId).call();
  return getTeamAddress;
}
