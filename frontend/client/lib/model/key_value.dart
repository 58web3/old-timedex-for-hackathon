...
class KeyValue {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "KeyValue[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "KeyValue[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<KeyValue>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <KeyValue>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = KeyValue.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, KeyValue> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = KeyValue.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<KeyValue>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<KeyValue>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = KeyValue.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
