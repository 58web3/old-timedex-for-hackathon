...
import 'mock_data_alice.dart';

class AliceHome extends HookConsumerWidget {
  AliceHome({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final aliceState = ref.watch(demoStateProvider);

    useEffectOnce(() {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        // アカウントタイル
        ref.watch(userAccountProvider.notifier).update((_) => aliceAccount);
        // カレンダーの予定
        ref
            .watch(userCalendarEventByTypeProvider.notifier)
            .update((_) => alicePersonalCalendar);
        ...
        if (aliceState == 2) {
          // 所属しているチーム
          ref.watch(teamIdsProvider.notifier).update((_) => aliceTeamIds);
          // 選択中のチーム
          ref
              .watch(selectedTeamIdProvider.notifier)
              .update((state) => aliceTeamIds.first);
          // 参加中のチームの詳細
          ref
              .watch(joiningTeamsProvider.notifier)
              .update((state) => joiningTeams);
          // 選択中のチームのチームメンバー
          for (final id in aliceTeamIds) {
            ref
                .watch(
                  teamMemberProvider(id).notifier,
                )
                .update((_) => aliceTeamMembers[id]!);
          }
        }
