...
class DialogAddTeam extends HookConsumerWidget {
  const DialogAddTeam({super.key});
            ...
            child: Column(
              children: [
                ...
                TeamDialogName(
                  width: width - 48,
                  onEdited: (text) {
                    ref.read(addTeamInfoProvider.notifier).update(
                          (state) => state.copyWith(name: text),
                        );
                  },
                ),
                Container(height: 12),
                TeamDialogDescription(
                  width: width - 48,
                  onEdited: (text) {
                    ref.read(addTeamInfoProvider.notifier).update(
                ...
                TeamDialogMember(
                  width: width - 48,
                  walletAddresses: ref.watch(addTeamInfoProvider).members,
                  onEdited: (wa) {
                    ref.read(addTeamInfoProvider.notifier).update(
                          (state) => state.copyWith(
                            members: {...state.members, wa},
                          ),
                        );
                  },
                  onDelete: (wa) {
          ...
          Positioned(
            ...
            child: Container(
              ...
              child: ElevatedButton(
                onPressed: () {
                  ref.read(teamInfoUseCaseProvider).createTeam();
